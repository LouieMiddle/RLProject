...
class Memory:
    ...
    def push(self, state, action, new_state, reward, done):
        # Fill the buffer with status from environment
...
def train(agent, env, num_episodes, batch_size, file2save, directory):
    ...
    for episode in range(num_episodes):
        ...
        while True:
            ...
            if done:
                print("Episode: ", episode + 1, " Reward: ", np.round(episode_reward, decimals=2), " Average reward: ", np.round(avg_reward, decimals=2))
    ...
    print("\n\n\nBest average reward in {:d}, episodes is: {:f}".format((episode + 1), np.round(best_avg_reward, decimals=2)))
